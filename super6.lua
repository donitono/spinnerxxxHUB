...
-- Auto Appraise Variables
flags['autoappraise'] = false -- NEW: Auto appraise fish
flags['appraisemode'] = 1 -- NEW: 1 = NPC Dialog, 2 = Anywhere (if available)
flags['appraisedelay'] = 1.0 -- NEW: Delay between appraisals
...
-- 🎣 AUTO FISHING LOOP SYSTEM - FULL AUTOMATION
flags['autofishingloop'] = false -- NEW: Master toggle for auto fishing loop
flags['autoequiprod'] = false -- NEW: Auto equip rod from inventory
flags['equipdelay'] = 0.5 -- NEW: Delay after equipping rod
flags['autocastloop'] = true -- Auto cast in loop (uses existing autocast)
flags['autoshakeloop'] = true -- Auto shake in loop (uses existing shake)
flags['autoreelloop'] = true -- Auto reel in loop (uses existing reel)
flags['autorestart'] = true -- Auto restart cycle if stuck
flags['restarttimeout'] = 10 -- Restart if no progress for X seconds
local autoFishingLoopActive = false
local autoEquipCooldown = 0
local lastEquipTime = 0
local lastProgressTime = 0
local selectedRodName = "" -- Rod to auto-equip
local preferredRods = {} -- List of preferred fishing rods

-- 🕐 SERVER AGE DETECTOR & OLD SERVER FINDER
flags['serveragechecker'] = false -- NEW: Enable server age detection
flags['autojoinoldserver'] = false -- NEW: Auto join old servers
flags['maxplayercount'] = 12 -- NEW: Max players in target server
flags['minserverage'] = 300 -- NEW: Minimum server age (5 minutes)
local currentServerAge = 0
local serverStartTime = 0
local serverHopCooldown = 0
local lastServerHop = 0
local foundOldServers = {} -- Cache of old servers
local serverMetrics = {
    playerCount = 0,
    serverAge = 0,
    averageFishRate = 0,
...
local serverHistory = {} -- Keep track of visited servers
...
-- 🐟 AUTO APPRAISE SYSTEM - AUTOMATIC FISH APPRAISAL
local function hasAppraisableFish()
    -- Check if player has fish in inventory that can be appraised
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj.Name:lower():find("appraiser") or obj.Name:lower():find("appraise") then
            if obj:FindFirstChild("HumanoidRootPart") then
                ...
                if distance < shortestDistance and distance < 20 then -- Within 20 studs
...
local function performNPCAppraise()
    -- Method 1: NPC Dialog Appraise
    local appraiser = findNearestAppraiser()
    if not appraiser then
        debugPrint("❌ [Auto Appraise] No appraiser NPC found nearby")
        return false
    end
    
    pcall(function()
        -- Start dialog with appraiser
        ...
        if dialogInteract then
            ...
            debugPrint("🐟 [Auto Appraise] Started dialog with appraiser")
            
            task.wait(0.5) -- Wait for dialog to open
            
            -- Request appraise UI
            ReplicatedStorage.packages.Net.RE.EventAppraiseService.RequestAppraiseUI:FireServer()
            debugPrint("🔍 [Auto Appraise] Requested appraise UI")
            
            task.wait(0.3)
            
            -- Start appraise process
            ReplicatedStorage.packages.Net.RE.EventAppraiseService.StartAppraise:FireServer()
            debugPrint("⚡ [Auto Appraise] Started appraise process")
            
            task.wait(1.0) -- Wait for appraise to complete
            
            -- Take appraised fish
            ReplicatedStorage.packages.Net.RE.EventAppraiseService.TakeNow:FireServer()
            debugPrint("✅ [Auto Appraise] Completed and taken fish")
...
local function performAppraiseAnywhere()
    -- Method 2: Appraise Anywhere (if available)
    pcall(function()
        ...
        if appraiseAnywhere and appraiseAnywhere.HaveValidFish then
            -- Check if we have valid fish
            local hasValidFish = appraiseAnywhere.HaveValidFish:InvokeServer()
            if hasValidFish then
                -- Fire appraise anywhere
                appraiseAnywhere.Fire:InvokeServer()
                debugPrint("⚡ [Auto Appraise] Used Appraise Anywhere")
                return true
            else
                debugPrint("❌ [Auto Appraise] No valid fish for Appraise Anywhere")
            end
        end
    end)
...
local function autoAppraise()
    ...
    if not hasAppraisableFish() then 
        debugPrint("🐟 [Auto Appraise] No appraisable fish found")
        return 
    end
    ...
    elseif flags['appraisemode'] == 2 then
        -- Appraise Anywhere Mode
        success = performAppraiseAnywhere()
    end
    
    if success then
        debugPrint("✅ [Auto Appraise] Fish appraisal completed successfully")
    else
        debugPrint("❌ [Auto Appraise] Fish appraisal failed")
    end
        ...
        playerGui.ChildAdded:Connect(function(gui)
            if flags['superinstantreel'] and gui.Name == "reel" then
                ...
                pcall(function()
                    ...
                    -- Fixed: ScreenGui doesn't have Visible property
                ...
                debugPrint("🚫 [ZERO-FLASH] Blocked reel GUI with no visual flash!")
            end
        end)
...
-- 🎣 AUTO FISHING LOOP FUNCTIONS - COMPLETE AUTOMATION SYSTEM
...
local function getRodsFromInventory()
    -- Get all fishing rods from player's backpack
    local rods = {}
    local backpack = lp.Backpack
    if not backpack then return rods end
    ...
    local equippedRod = FindRod()
    if equippedRod then
        table.insert(rods, equippedRod.Name)
    end
...
local function findBestRod()
    -- Find the best available fishing rod to equip
    local backpack = lp.Backpack
    if not backpack then return nil end
    
    -- Priority order for rod selection (best to worst)
    local rodPriority = {
        "Aurora Rod", "Mythical Rod", "Mythical", "Aurora", "Kraken Rod", "Kraken", 
        "Leviathan Rod", "Leviathan", "Megalodon Rod", "Megalodon", "No-Life Rod", "No-life", "Nolife",
        "Crystal Rod", "Crystal", "Fortune Rod", "Fortune", "Steady Rod", "Steady",
        "Fast Rod", "Fast", "Lucky Rod", "Lucky", "Long Rod", "Long",
        "Carbon Rod", "Carbon", "Reinforced Rod", "Reinforced", "Kings Rod", "Kings", "King",
        "Nocturnal Rod", "Nocturnal", "Rapid Rod", "Rapid", "Magnet Rod", "Magnet",
        "Training Rod", "Training", "Plastic Rod", "Plastic", "Flimsy Rod", "Flimsy"
    }
    
    -- If user has selected a specific rod, prefer that
    if selectedRodName and selectedRodName ~= "" then
        local specificRod = backpack:FindFirstChild(selectedRodName)
        if specificRod then
            return specificRod
        end
    end
    
    -- Find highest priority rod available
    for _, rodName in pairs(rodPriority) do
        for _, item in pairs(backpack:GetChildren()) do
            if item:IsA("Tool") and item.Name:find(rodName) then
                return item
            end
        end
    end
...
local function equipRod(rodTool)
    -- Equip a fishing rod tool
    if not rodTool then return false end
    
    pcall(function()
        -- Method 1: Direct equip via RemoteEvent (preferred)
        local equipRemote = ReplicatedStorage.packages.Net.RE["Rod/Equip"]
        if equipRemote then
            equipRemote:FireServer(rodTool.Name)
            debugPrint("🎣 [Auto Equip] Equipped rod via RemoteEvent: " .. rodTool.Name)
        end
        
        -- Method 2: Direct tool equip (backup)
        local humanoid = gethum()
        if humanoid then
            humanoid:EquipTool(rodTool)
            debugPrint("🎣 [Auto Equip] Equipped rod via Humanoid: " .. rodTool.Name)
        end
        
        -- Method 3: Tool parent change (fallback)
        rodTool.Parent = getchar()
        debugPrint("🎣 [Auto Equip] Equipped rod via Parent: " .. rodTool.Name)
    end)
...
local function autoEquipBestRod()
    -- Main function to automatically equip the best available rod
    if hasRodEquipped() then
        return true -- Already have rod equipped
    end
    
    -- Check cooldown to prevent spam
    local currentTime = tick()
    if currentTime - lastEquipTime < flags['equipdelay'] then
        return false
    end
    
    -- Find and equip best rod
    local bestRod = findBestRod()
    if bestRod then
        if equipRod(bestRod) then
            lastEquipTime = currentTime
            debugPrint("🎣 [Auto Fishing Loop] Auto-equipped rod: " .. bestRod.Name)
            return true
        end
    else
        debugPrint("❌ [Auto Fishing Loop] No fishing rods found in inventory!")
        return false
    end
...
-- 🕐 SERVER AGE DETECTOR SYSTEM - FIND OLD SERVERS FOR BETTER FISHING
local TeleportService = game:GetService("TeleportService")
local HttpService = game:GetService("HttpService")

local function calculateServerAge()
    -- Calculate current server age in seconds
    local currentTime = workspace.DistributedGameTime
    if serverStartTime == 0 then
        serverStartTime = currentTime
    end
    currentServerAge = currentTime - serverStartTime
    return currentServerAge
end

local function getServerMetrics()
    -- Get comprehensive server information
    local metrics = {
        playerCount = #Players:GetPlayers(),
        serverAge = calculateServerAge(),
        gameId = game.GameId,
        placeId = game.PlaceId,
        jobId = game.JobId
    }
    
    -- Calculate fish rate (based on recent fishing activity)
    local fishCount = 0
    pcall(function()
        -- Count fish in player's inventory as indicator
        local character = lp.Character
        if character then
            local backpack = lp.Backpack
            if backpack then
    ...
    metrics.fishCount = fishCount
    metrics.averageFishRate = metrics.serverAge > 0 and (fishCount / (metrics.serverAge / 60)) or 0
    
    -- Check event activity (more events = older/more stable server)
    metrics.eventActivity = 0
    pcall(function()
        if workspace:FindFirstChild("active") then
            metrics.eventActivity = #workspace.active:GetChildren()
        end
    end)
...
local function isGoodOldServer(metrics)
    -- Determine if server meets "old server" criteria
    local criteria = {
        playerCount = metrics.playerCount <= flags['maxplayercount'],
        serverAge = metrics.serverAge >= flags['minserverage'],
        fishRate = metrics.averageFishRate > 0.5, -- At least 0.5 fish per minute
        eventActivity = metrics.eventActivity >= 2 -- At least 2 active events
    }
    
    local score = 0
    local totalCriteria = 0
    
    for criterion, passes in pairs(criteria) do
    ...
    -- Server is "good old" if it meets at least 3 out of 4 criteria
    return score >= 3, score, totalCriteria
end

local function getServerList()
    -- Get list of available servers (simplified version)
    local serverList = {}
    
    -- Note: This is a basic implementation. Real server hopping would need more complex methods
    pcall(function()
        -- Try to get servers from Roblox API (if available)
...
local function joinOldServer()
    -- Attempt to join an old server
    if tick() - lastServerHop < 30 then -- 30 second cooldown
        debugPrint("⏰ [Server Hop] Cooldown active, wait " .. (30 - (tick() - lastServerHop)) .. "s")
        return false
    end
    ...
    -- Find best old server from list
    for _, server in pairs(serverList) do
        if server.playerCount <= flags['maxplayercount'] and server.playerCount > 1 then
            local score = (server.maxPlayers - server.playerCount) + (server.ping < 100 and 5 or 0)
    ...
    -- Try to teleport to target server
    if targetServer then
        pcall(function()
            lastServerHop = tick()
            debugPrint("🚀 [Server Hop] Joining server with " .. targetServer.playerCount .. " players...")
            TeleportService:TeleportToPlaceInstance(game.PlaceId, targetServer.id, lp)
        end)
        return true
    else
        debugPrint("❌ [Server Hop] No suitable old servers found")
        return false
    end
end
...
local function displayServerInfo()
    ...
    local metrics = getServerMetrics()
    local isGood, score, total = isGoodOldServer(metrics)
    
    debugPrint("📊 [Server Info] Current Server Analysis:")
    debugPrint("  👥 Players: " .. metrics.playerCount .. " (Target: ≤" .. flags['maxplayercount'] .. ")")
    debugPrint("  🕐 Age: " .. math.floor(metrics.serverAge) .. "s (" .. math.floor(metrics.serverAge/60) .. "m)")
    debugPrint("  🐟 Fish Rate: " .. string.format("%.1f", metrics.averageFishRate) .. " fish/min")
    debugPrint("  ⭐ Events: " .. metrics.eventActivity .. " active")
    debugPrint("  📈 Score: " .. score .. "/" .. total .. " " .. (isGood and "✅ GOOD OLD SERVER" or "❌ Not optimal"))
    debugPrint("  🆔 Job ID: " .. string.sub(metrics.jobId, 1, 8) .. "...")
end

local function addServerToHistory(metrics, reason)
    -- Add server to history with timestamp
    local serverRecord = {
        jobId = metrics.jobId,
        timestamp = tick(),
        playerCount = metrics.playerCount,
        serverAge = metrics.serverAge,
        fishRate = metrics.averageFishRate,
        eventActivity = metrics.eventActivity,
        reason = reason or "manual"
    }
    
    table.insert(serverHistory, serverRecord)
    
    -- Keep only last 10 servers in history
    if #serverHistory > 10 then
        table.remove(serverHistory, 1)
    end
    
    -- Update session stats
    currentSessionStats.serversVisited = currentSessionStats.serversVisited + 1
    
    local _, score, _ = isGoodOldServer(metrics)
    if score > currentSessionStats.bestServerScore then
        currentSessionStats.bestServerScore = score
    end
end

local function displaySessionStats()
    -- Display session statistics
    local sessionTime = tick() - currentSessionStats.sessionStartTime
    
    debugPrint("📈 [Session Stats] Server Hopping Session:")
    debugPrint("  ⏱️ Session Time: " .. math.floor(sessionTime/60) .. " minutes")
    debugPrint("  🚀 Servers Visited: " .. currentSessionStats.serversVisited)
    debugPrint("  🏆 Best Server Score: " .. currentSessionStats.bestServerScore .. "/4")
    debugPrint("  🐟 Total Fish: " .. currentSessionStats.totalFishCaught)
    debugPrint("  📊 Avg Fish/Server: " .. (currentSessionStats.serversVisited > 0 and 
                math.floor(currentSessionStats.totalFishCaught / currentSessionStats.serversVisited) or 0))
end

local function displayServerHistory()
    -- Display recent server history
    debugPrint("📋 [Server History] Recent Servers (Last " .. #serverHistory .. "):")
    
    for i, server in pairs(serverHistory) do
        local timeAgo = math.floor((tick() - server.timestamp) / 60)
        debugPrint("  " .. i .. ". [" .. timeAgo .. "m ago] " .. 
                   server.playerCount .. " players, " ..
                   math.floor(server.serverAge/60) .. "m old, " ..
                   string.format("%.1f", server.fishRate) .. " fish/min (" .. server.reason .. ")")
    end
end

-- Initialize session stats
if currentSessionStats.sessionStartTime == 0 then
    currentSessionStats.sessionStartTime = tick()
end

--[[
=================================================================
� PLAYER TELEPORT SYSTEM - TELEPORT TO OTHER PLAYERS
=================================================================
Comprehensive player teleport system that allows teleporting
to any player in the current server.

Features:
- Real-time player list with search functionality
- Instant teleport to player positions
- Distance calculation and safety checks
- Player tracking and follow system
...
flags['playerteleport'] = false -- Enable player teleport system
flags['followplayer'] = false -- Follow selected player
flags['teleportsafety'] = true -- Enable safety checks
flags['maxtelportdistance'] = 10000 -- Maximum teleport distance
local selectedPlayerName = "" -- Currently selected player for teleport
local currentFollowTarget = nil -- Player being followed
local followConnection = nil -- Follow update connection
local playerRefreshRate = 2 -- Seconds between player list updates
local lastPlayerRefresh = 0
local cachedPlayerList = {} -- Cached player list for UI

-- 👥 PLAYER TELEPORT FUNCTIONS
local function getAllPlayers()
    -- Get all players in the current server
    local playerList = {}
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= lp and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
                distance = (lp.Character.HumanoidRootPart.Position - humanoidRootPart.Position).Magnitude
            end
...
local function searchPlayersByName(searchTerm)
    -- Search players by name or display name
    ...
    for _, playerData in pairs(allPlayers) do
        ...
        local displayName = playerData.displayName:lower()
        
        if name:find(searchTerm) or displayName:find(searchTerm) then
...
local function getPlayerByName(playerName)
    ...
    for _, playerData in pairs(allPlayers) do
        if playerData.name == playerName or playerData.displayName == playerName then
            return playerData
        end
    end
...
local function validateTeleportSafety(targetPosition)
    -- Validate teleport safety
    if not flags['teleportsafety'] then
        return true, "Safety checks disabled"
    end
    
    if not lp.Character or not lp.Character:FindFirstChild("HumanoidRootPart") then
        return false, "Local player character not found"
    end
    
    local currentPosition = lp.Character.HumanoidRootPart.Position
    local distance = (targetPosition - currentPosition).Magnitude
    
    if distance > flags['maxtelportdistance'] then
        return false, "Teleport distance too far: " .. math.floor(distance) .. " studs (max: " .. flags['maxtelportdistance'] .. ")"
    end
    
    -- Check if target position is in void (too far below map)
    if targetPosition.Y < -1000 then
        return false, "Target position appears to be in void"
    end
    
    return true, "Teleport safe"
end

local function teleportToPlayer(targetPlayer)
    -- Teleport to specific player
    if not targetPlayer then
        debugPrint("❌ [Player Teleport] Target player not specified")
        return false
    end
    ...
    if type(targetPlayer) == "string" then
        playerData = getPlayerByName(targetPlayer)
        if not playerData then
            debugPrint("❌ [Player Teleport] Player '" .. targetPlayer .. "' not found")
            return false
        end
    else
    ...
    if not playerData.character or not playerData.character:FindFirstChild("HumanoidRootPart") then
        debugPrint("❌ [Player Teleport] Player character not found: " .. playerData.name)
        return false
    end
    
    local targetPosition = playerData.position
    local isValid, reason = validateTeleportSafety(targetPosition)
    
    if not isValid then
        debugPrint("❌ [Player Teleport] Safety check failed: " .. reason)
        return false
    end
    
    if not lp.Character or not lp.Character:FindFirstChild("HumanoidRootPart") then
        debugPrint("❌ [Player Teleport] Local player character not found")
        return false
    end
    ...
    if success then
        debugPrint("✅ [Player Teleport] Successfully teleported to " .. playerData.name .. " (Distance: " .. playerData.distance .. " studs)")
        selectedPlayerName = playerData.name
        return true
    else
        debugPrint("❌ [Player Teleport] Failed to teleport to " .. playerData.name)
        return false
    end
end

local function startFollowPlayer(targetPlayerName)
    ...
    if not targetPlayerName or targetPlayerName == "" then
        debugPrint("❌ [Player Follow] No target player specified")
        return false
    end
    
    local targetPlayerData = getPlayerByName(targetPlayerName)
    if not targetPlayerData then
        debugPrint("❌ [Player Follow] Target player not found: " .. targetPlayerName)
        return false
    end
    
    currentFollowTarget = targetPlayerData.player
    
    debugPrint("🏃 [Player Follow] Started following " .. targetPlayerName)
    
    followConnection = RunService.Heartbeat:Connect(function()
        if not flags['followplayer'] then
            return
        end
        
        if not currentFollowTarget or not currentFollowTarget.Parent then
            stopFollowPlayer()
            return
        end
        
        local targetCharacter = currentFollowTarget.Character
        ...
        -- Follow if distance is greater than 10 studs
        if distance > 10 then
            local isValid, _ = validateTeleportSafety(targetPosition)
            if isValid then
                lp.Character.HumanoidRootPart.CFrame = CFrame.new(targetPosition + Vector3.new(math.random(-3, 3), 0, math.random(-3, 3)))
            end
        end
    end)
...
local function stopFollowPlayer()
    ...
    if currentFollowTarget then
        debugPrint("⏹️ [Player Follow] Stopped following " .. currentFollowTarget.Name)
        currentFollowTarget = nil
    end
    
    flags['followplayer'] = false
end

local function getPlayerListForUI()
    -- Get formatted player list for UI dropdown
    local currentTime = tick()
    
    -- Update cached list if needed
    if currentTime - lastPlayerRefresh > playerRefreshRate then
        cachedPlayerList = {}
        local players = getAllPlayers()
        
        table.insert(cachedPlayerList, "Select Player...")
        
        for _, playerData in pairs(players) do
            local displayText = playerData.name .. " (" .. playerData.distance .. " studs)"
            if playerData.displayName ~= playerData.name then
                displayText = playerData.displayName .. " (" .. playerData.name .. ") - " .. playerData.distance .. " studs"
            end
            table.insert(cachedPlayerList, displayText)
        end
        
        lastPlayerRefresh = currentTime
    end
    
    return cachedPlayerList
end

local function extractPlayerNameFromUIText(uiText)
    -- Extract actual player name from UI display text
    if not uiText or uiText == "Select Player..." then
        return nil
    end
    
    -- Handle format: "DisplayName (ActualName) - XXX studs"
    local actualName = uiText:match("%(([^%)]+)%)")
    if actualName then
        return actualName
    end
    
    -- Handle format: "PlayerName (XXX studs)"
    local playerName = uiText:match("^([^%(]+)")
    if playerName then
        return playerName:gsub("%s+$", "") -- Remove trailing spaces
    end
...
--[[
=================================================================
�🛒 AUTO BUY ROD SYSTEM - COMPREHENSIVE ROD PURCHASING
=================================================================
Complete rod purchasing system that allows buying rods directly 
without needing to visit their locations.

Features:
- Complete rod database with prices & locations
- Auto teleport & purchase system
- Bulk rod purchasing options
- Smart error handling & retries
- Integration with existing shop system
=================================================================
]]--

-- Rod Purchase Variables
local rodPurchaseFlags = {
    selectedRod = "",
    purchaseQuantity = 1,
...
-- Comprehensive Rod Database
local rodDatabase = {
    -- Legendary/Mythical Rods
    ["No-Life Rod"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Ultimate fishing rod" },
    ["Rod Of The Depths"] = { price = 0, location = CFrame.new(-1045, 200, -1100), currency = "C$", category = "Quest", description = "Deep sea specialist" },
    ["Rod Of The Eternal King"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Royal fishing rod" },
    ["Rod Of The Forgotten Fang"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Ancient power" },
    ["Leviathan's Fang Rod"] = { price = 0, location = CFrame.new(-1045, 200, -1100), currency = "C$", category = "Quest", description = "Beast hunter" },
    
    -- High Tier Rods
    ["Mythical Rod"] = { price = 7500, location = CFrame.new(229, 139, 42), currency = "C$", category = "Premium", description = "Mythical power rod" },
    ["Aurora Rod"] = { price = 5000, location = CFrame.new(229, 139, 42), currency = "C$", category = "Premium", description = "Northern lights rod" },
    ["Kraken Rod"] = { price = 4500, location = CFrame.new(-175, 145, 1935), currency = "C$", category = "Premium", description = "Sea monster hunter" },
    ["Poseidon Rod"] = { price = 4000, location = CFrame.new(-1045, 200, -1100), currency = "C$", category = "Premium", description = "Ocean lord's rod" },
    ["Zeus Rod"] = { price = 6000, location = CFrame.new(6075, 195, 260), currency = "C$", category = "Premium", description = "Thunder god's rod" },
    
    -- Special/Event Rods  
    ["Heaven's Rod"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Event", description = "Divine fishing rod" },
    ["Celestial Rod"] = { price = 8000, location = CFrame.new(6075, 195, 260), currency = "C$", category = "Premium", description = "Star-powered rod" },
    ["Ethereal Prism Rod"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Prismatic power" },
    ["Rod Of The Zenith"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Peak performance" },
    ["Challenger's Rod"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Event", description = "Tournament winner" },
    
    -- Mid Tier Rods
    ["Sunken Rod"] = { price = 2500, location = CFrame.new(-1045, 200, -1100), currency = "C$", category = "Standard", description = "Underwater specialist" },
    ["Tempest Rod"] = { price = 3000, location = CFrame.new(-175, 145, 1935), currency = "C$", category = "Standard", description = "Storm catcher" },
    ["Trident Rod"] = { price = 3500, location = CFrame.new(-1045, 200, -1100), currency = "C$", category = "Standard", description = "Three-pronged power" },
    ["Kings Rod"] = { price = 5000, location = CFrame.new(229, 139, 42), currency = "C$", category = "Premium", description = "Royal quality" },
    ["Destiny Rod"] = { price = 4500, location = CFrame.new(229, 139, 42), currency = "C$", category = "Premium", description = "Fate's favorite" },
    
    -- Standard Rods
    ["Carbon Rod"] = { price = 500, location = CFrame.new(229, 139, 42), currency = "C$", category = "Basic", description = "Lightweight carbon" },
    ["Reinforced Rod"] = { price = 350, location = CFrame.new(229, 139, 42), currency = "C$", category = "Basic", description = "Extra durability" },
    ["Lucky Rod"] = { price = 750, location = CFrame.new(229, 139, 42), currency = "C$", category = "Basic", description = "Fortune's friend" },
    ["Fast Rod"] = { price = 400, location = CFrame.new(229, 139, 42), currency = "C$", category = "Basic", description = "Quick caster" },
    ["Long Rod"] = { price = 600, location = CFrame.new(229, 139, 42), currency = "C$", category = "Basic", description = "Extended reach" },
    
    -- Utility Rods
    ["Magnet Rod"] = { price = 1200, location = CFrame.new(229, 139, 42), currency = "C$", category = "Utility", description = "Magnetic attraction" },
    ["Precision Rod"] = { price = 1500, location = CFrame.new(229, 139, 42), currency = "C$", category = "Utility", description = "Perfect accuracy" },
    ["Wisdom Rod"] = { price = 2000, location = CFrame.new(229, 139, 42), currency = "C$", category = "Utility", description = "Knowledge seeker" },
    ["Resourceful Rod"] = { price = 1800, location = CFrame.new(229, 139, 42), currency = "C$", category = "Utility", description = "Resource gatherer" },
    
    -- Starter Rods
    ["Training Rod"] = { price = 50, location = CFrame.new(229, 139, 42), currency = "C$", category = "Starter", description = "Beginner friendly" },
    ["Flimsy Rod"] = { price = 25, location = CFrame.new(229, 139, 42), currency = "C$", category = "Starter", description = "Basic starter rod" },
    ["Steady Rod"] = { price = 100, location = CFrame.new(229, 139, 42), currency = "C$", category = "Starter", description = "Reliable performance" },
    
    -- Special/Unique Rods
    ["Electric Guitar"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Special", description = "Rock & fishing" },
    ["Volcanic Rod"] = { price = 3200, location = CFrame.new(6075, 195, 260), currency = "C$", category = "Special", description = "Lava-powered" },
    ["Arctic Rod"] = { price = 2800, location = CFrame.new(-175, 145, 1935), currency = "C$", category = "Special", description = "Ice fishing specialist" },
    ["Phoenix Rod"] = { price = 4800, location = CFrame.new(6075, 195, 260), currency = "C$", category = "Special", description = "Reborn from ashes" },
    ["Carrot Rod"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Event", description = "Vegetable power" },
    
    -- Quest/Achievement Rods
    ["Polaris Serenade"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Northern star song" },
    ["Onirifalx"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Dream cutter" },
    ["Duskwire"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Twilight fisher" },
    ["Illumina"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Light bringer" },
    ["Spiritbinder"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Soul catcher" },
    
    -- Additional Rods from Data
    ["The Lost Rod"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Lost in time" },
    ["Voyager Rod"] = { price = 2200, location = CFrame.new(-175, 145, 1935), currency = "C$", category = "Standard", description = "Explorer's choice" },
    ["Riptide Rod"] = { price = 2800, location = CFrame.new(-1045, 200, -1100), currency = "C$", category = "Standard", description = "Current rider" },
    ["Abyssal Specter Rod"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Ghostly depths" },
    ["ReRod"] = { price = 1500, location = CFrame.new(229, 139, 42), currency = "C$", category = "Standard", description = "Recycled power" },
    ["Mila's Wand Of Magic"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Event", description = "Magical fishing" },
    ["Developers Rod"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Special", description = "Dev exclusive" },
    ["Rod Of The Exalted One"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Divine blessing" },
    ["Champions Rod"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Event", description = "Winner's prize" },
    ["Depthseeker Rod"] = { price = 3800, location = CFrame.new(-1045, 200, -1100), currency = "C$", category = "Premium", description = "Deep explorer" },
    ["Seraphic Rod"] = { price = 9000, location = CFrame.new(6075, 195, 260), currency = "C$", category = "Premium", description = "Angelic power" },
    ["Blazebringer Rod"] = { price = 4200, location = CFrame.new(6075, 195, 260), currency = "C$", category = "Special", description = "Fire wielder" },
    ["Verdant Shear Rod"] = { price = 2600, location = CFrame.new(-175, 145, 1935), currency = "C$", category = "Special", description = "Nature's blade" },
    ["Wildflower Rod"] = { price = 1400, location = CFrame.new(-175, 145, 1935), currency = "C$", category = "Standard", description = "Garden beauty" },
    ["Azure Of Lagoon"] = { price = 3600, location = CFrame.new(-1045, 200, -1100), currency = "C$", category = "Premium", description = "Blue depths" },
    ["Frog Rod"] = { price = 800, location = CFrame.new(-175, 145, 1935), currency = "C$", category = "Fun", description = "Amphibian ally" },
    ["Firefly Rod"] = { price = 1600, location = CFrame.new(-175, 145, 1935), currency = "C$", category = "Standard", description = "Night light" },
    ["Adventurer's Rod"] = { price = 1000, location = CFrame.new(229, 139, 42), currency = "C$", category = "Standard", description = "Explorer's gear" },
    ["Great Dreamer Rod"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Sleepy power" },
    ["Free Spirit Rod"] = { price = 1800, location = CFrame.new(229, 139, 42), currency = "C$", category = "Standard", description = "Independent spirit" },
    ["Wingripper"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Wing tearer" },
    ["Tryhard Rod"] = { price = 2400, location = CFrame.new(229, 139, 42), currency = "C$", category = "Standard", description = "Serious fisherman" },
    ["Great Rod of Oscar"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Oscar's legacy" },
    ["Astralhook Rod"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Cosmic catcher" },
    ["nilCaster"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Special", description = "Void fisher" },
    ["Starline Caster"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Star line" },
    ["Fallen Snowblade"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Icy blade" },
    ["Magma Rod"] = { price = 3400, location = CFrame.new(6075, 195, 260), currency = "C$", category = "Special", description = "Molten core" },
    ["Fang of the Eclipse"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Eclipse fang" },
    ["Axe of Rhoads"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Axe wielder" },
    ["Summit Rod"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Peak climber" },
    ["???"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Mystery", description = "Unknown power" },
    ["Eardrum"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Quest", description = "Sound catcher" },
    ["Scurvy Rod"] = { price = 2200, location = CFrame.new(-1045, 200, -1100), currency = "C$", category = "Standard", description = "Pirate's tool" },
    ["Rainbow Cluster Rod"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Event", description = "Colorful power" },
    ["Silly Fun Happy Rod"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Fun", description = "Joy bringer" },
    ["Popsicle Rod"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Event", description = "Sweet treat" },
    ["Paper Fan Rod"] = { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Fun", description = "Cooling breeze" }
}

-- Rod Purchase Functions
local function getRodCategories()
    local categories = {}
    for rodName, rodData in pairs(rodDatabase) do
        if not table.find(categories, rodData.category) then
            table.insert(categories, rodData.category)
        end
    end
...
local function getRodsByCategory(category)
    local rods = {}
    for rodName, rodData in pairs(rodDatabase) do
...
local function getAllPurchasableRods()
    local rods = {}
    for rodName, rodData in pairs(rodDatabase) do
        if rodData.price > 0 then -- Only include purchasable rods
            table.insert(rods, rodName)
        end
    end
    table.sort(rods)
...
local function getRodInfo(rodName)
    return rodDatabase[rodName] or { price = 0, location = CFrame.new(0, 0, 0), currency = "C$", category = "Unknown", description = "No data available" }
end

local function formatPrice(price)
    if price == 0 then
        return "Quest/Event Only"
    elseif price >= 1000000 then
        return string.format("%.1fM C$", price / 1000000)
    elseif price >= 1000 then
        return string.format("%.1fK C$", price / 1000)
    else
        return price .. " C$"
    end
end

local function purchaseRod(rodName, quantity)
    -- Main rod purchasing function
    quantity = quantity or 1
    local rodInfo = getRodInfo(rodName)
    
    if rodInfo.price == 0 then
        debugPrint("❌ [Rod Purchase] " .. rodName .. " cannot be purchased (Quest/Event only)")
        return false
    end
    
    debugPrint("🛒 [Rod Purchase] Attempting to purchase " .. quantity .. "x " .. rodName .. " for " .. formatPrice(rodInfo.price * quantity))
    
    rodPurchaseFlags.purchaseInProgress = true
    
    -- Teleport to rod location
    if lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
        debugPrint("📍 [Rod Purchase] Teleporting to " .. rodName .. " location...")
        lp.Character.HumanoidRootPart.CFrame = rodInfo.location
        ...
        pcall(function()
            -- Method 1: Rod Shop RemoteEvent
            local rodShopRemote = ReplicatedStorage:FindFirstChild("packages")
            if rodShopRemote and rodShopRemote:FindFirstChild("Net") then
                local purchaseRemote = rodShopRemote.Net:FindFirstChild("RE/Shop/Purchase")
                if purchaseRemote then
                    for i = 1, quantity do
                        purchaseRemote:FireServer(rodName, "Rod")
                        wait(0.5)
                    end
                    purchaseSuccess = true
                    debugPrint("✅ [Rod Purchase] Method 1 successful: " .. rodName)
                end
            end
        end)
        
        -- Method 2: Direct shop interaction
        if not purchaseSuccess then
            pcall(function()
                local shopRemote = ReplicatedStorage.packages.Net:FindFirstChild("RE/BuyRod/Purchase")
                if shopRemote then
                    for i = 1, quantity do
                        shopRemote:FireServer(rodName, quantity)
                        wait(0.5)
                    end
                    purchaseSuccess = true
                    debugPrint("✅ [Rod Purchase] Method 2 successful: " .. rodName)
                end
            end)
        end
        
        -- Method 3: General purchase system
        if not purchaseSuccess then
            pcall(function()
                local generalPurchase = ReplicatedStorage.packages.Net:FindFirstChild("RE/Purchase")
                if generalPurchase then
                    for i = 1, quantity do
                        generalPurchase:FireServer("Rod", rodName, rodInfo.price)
                        wait(0.5)
                    end
                    purchaseSuccess = true
                    debugPrint("✅ [Rod Purchase] Method 3 successful: " .. rodName)
                end
            end)
        end
        
        -- Method 4: NPC interaction based purchase
        if not purchaseSuccess then
            pcall(function()
                -- Look for nearby shop NPCs
                for _, npc in pairs(workspace:GetDescendants()) do
                    if npc.Name:lower():find("shop") or npc.Name:lower():find("merchant") or npc.Name:lower():find("seller") then
                        if npc:FindFirstChild("HumanoidRootPart") then
                            local distance = (lp.Character.HumanoidRootPart.Position - npc.HumanoidRootPart.Position).Magnitude
                            if distance < 20 then -- Within 20 studs
                                local npcInteract = ReplicatedStorage.packages.Net:FindFirstChild("RE/NPC/Purchase")
                                if npcInteract then
                                    for i = 1, quantity do
                                        npcInteract:FireServer(npc, rodName, "Rod")
                                        wait(0.5)
                                    end
                                    purchaseSuccess = true
                                    debugPrint("✅ [Rod Purchase] Method 4 successful via NPC: " .. rodName)
                                    break
                                end
                            end
                        end
        ...
        if purchaseSuccess then
            rodPurchaseFlags.totalPurchased = rodPurchaseFlags.totalPurchased + quantity
            debugPrint("🎉 [Rod Purchase] Successfully purchased " .. quantity .. "x " .. rodName .. "!")
            debugPrint("📊 [Rod Purchase] Total rods purchased this session: " .. rodPurchaseFlags.totalPurchased)
        else
            debugPrint("❌ [Rod Purchase] Failed to purchase " .. rodName .. " - trying alternative methods...")
            
            -- Final fallback: Try game-specific purchase events
            pcall(function()
                -- Look for game-specific purchase events
                for _, remote in pairs(ReplicatedStorage:GetDescendants()) do
                    if remote:IsA("RemoteEvent") and (
                        remote.Name:lower():find("purchase") or 
                        remote.Name:lower():find("buy") or 
                        remote.Name:lower():find("shop")
                    ) then
                        remote:FireServer(rodName, quantity)
                        wait(0.5)
                        debugPrint("🔄 [Rod Purchase] Tried fallback method: " .. remote.Name)
                    end
                end
            end)
        end
    ...
    rodPurchaseFlags.purchaseInProgress = false
    return purchaseSuccess or false
end

local function bulkPurchaseRods(rodList, quantity)
    -- Purchase multiple rods in sequence
    local successCount = 0
    local totalRods = #rodList
    
    debugPrint("🛒 [Bulk Purchase] Starting bulk purchase of " .. totalRods .. " rod types...")
    
    for i, rodName in pairs(rodList) do
        if rodPurchaseFlags.autoPurchaseEnabled then
            debugPrint("🔄 [Bulk Purchase] Processing " .. i .. "/" .. totalRods .. ": " .. rodName)
            
            if purchaseRod(rodName, quantity) then
                successCount = successCount + 1
            end
            
            wait(1) -- Delay between purchases to avoid rate limiting
        else
            debugPrint("⏹️ [Bulk Purchase] Cancelled by user")
            break
        end
    end
    
    debugPrint("🏁 [Bulk Purchase] Completed! Successfully purchased " .. successCount .. "/" .. totalRods .. " rod types")
    return successCount
end

-- Enhanced notification system for rod purchases
local function showRodPurchaseNotification(rodName, success, reason)
    local notificationText = ""
    local notificationTime = 3
    
    if success then
        notificationText = "✅ Successfully purchased: " .. rodName .. "!"
        notificationTime = 2
    else
        notificationText = "❌ Failed to purchase: " .. rodName .. " (" .. (reason or "unknown error") .. ")"
        notificationTime = 4
    end
    
    -- Use the existing message function if available
    if message then
        message(notificationText, notificationTime)
    end
    
    debugPrint(notificationText)
end

-- Advanced rod search and filtering
local function searchRodsByName(searchTerm)
    local results = {}
    local lowerSearchTerm = searchTerm:lower()
    
    for rodName, rodData in pairs(rodDatabase) do
        if rodName:lower():find(lowerSearchTerm) then
            table.insert(results, {
                name = rodName,
                data = rodData,
                relevanceScore = rodName:lower():find(lowerSearchTerm) == 1 and 10 or 5 -- Exact match gets higher score
            })
        end
    end
    
    -- Sort by relevance score
    table.sort(results, function(a, b) return a.relevanceScore > b.relevanceScore end)
    
    return results
end

-- Rod recommendation system
local function getRecommendedRods(playerLevel, budget)
    local recommendations = {}
    playerLevel = playerLevel or 1
    budget = budget or 1000
    
    for rodName, rodData in pairs(rodDatabase) do
        if rodData.price > 0 and rodData.price <= budget then
            local score = 0
            
            -- Score based on price efficiency
            if rodData.price < budget * 0.3 then
                score = score + 3 -- Affordable
            elseif rodData.price < budget * 0.7 then
                score = score + 2 -- Reasonable
            else
                score = score + 1 -- Expensive but within budget
            end
            
            -- Score based on category
            if rodData.category == "Premium" then
                score = score + 3
            elseif rodData.category == "Standard" then
                score = score + 2
            elseif rodData.category == "Basic" then
                score = score + 1
            end
            
            -- Bonus for utility rods
            if rodData.category == "Utility" then
                score = score + 2
            end
...
-- Player currency checker (if possible)
local function getPlayerCurrency()
    -- Try to get player's current currency
    local currency = 0
    
    pcall(function()
        -- Check various common currency locations
        if lp.leaderstats and lp.leaderstats:FindFirstChild("C$") then
            currency = lp.leaderstats["C$"].Value
        elseif lp.leaderstats and lp.leaderstats:FindFirstChild("Cash") then
            currency = lp.leaderstats.Cash.Value
        elseif lp.leaderstats and lp.leaderstats:FindFirstChild("Money") then
            currency = lp.leaderstats.Money.Value
        end
    end)
...
local function validatePurchase(rodName, quantity)
    local rodInfo = getRodInfo(rodName)
    local totalCost = rodInfo.price * quantity
    ...
    -- Check if rod exists
    if not rodDatabase[rodName] then
        validation.valid = false
        table.insert(validation.errors, "Rod does not exist in database")
        return validation
    end
    
    -- Check if rod is purchasable
    if rodInfo.price == 0 then
        validation.valid = false
        table.insert(validation.errors, "Rod is quest/event only - cannot be purchased")
        return validation
    end
    
    -- Check currency (if we can detect it)
    if playerCurrency > 0 and totalCost > playerCurrency then
        validation.valid = false
        table.insert(validation.errors, "Insufficient currency: need " .. formatPrice(totalCost) .. ", have " .. formatPrice(playerCurrency))
        return validation
    end
    
    -- Add warnings for expensive purchases
    if totalCost > 10000 then
        table.insert(validation.warnings, "This is an expensive purchase: " .. formatPrice(totalCost))
    end
    
    if quantity > 5 then
        table.insert(validation.warnings, "Purchasing multiple copies of the same rod")
    end
...
if Shop then
    ...
    Shop.createShopTab = function(self, Window)
        ...
        -- 🛒 AUTO BUY ROD SYSTEM UI
        local RodShopSection = ShopTab:NewSection("🎣 Auto Buy Rods")
        
        RodShopSection:NewLabel("Complete Rod Purchasing System")
        RodShopSection:NewLabel("📊 Total Rods Available: " .. #getAllPurchasableRods() .. " purchasable")
        RodShopSection:NewLabel("🎯 Categories: " .. table.concat(getRodCategories(), ", "))
        
        -- Rod selection dropdown
        local allRods = getAllPurchasableRods()
        RodShopSection:NewDropdown("Select Rod", "Choose rod to purchase", allRods, function(selectedRod)
            rodPurchaseFlags.selectedRod = selectedRod
            local rodInfo = getRodInfo(selectedRod)
            debugPrint("🎣 [Rod Shop] Selected: " .. selectedRod .. " - " .. formatPrice(rodInfo.price))
            debugPrint("📝 [Rod Shop] " .. rodInfo.description .. " (" .. rodInfo.category .. ")")
        end)
        
        -- Category filter dropdown
        local categories = {"All"}
        for _, cat in pairs(getRodCategories()) do
            table.insert(categories, cat)
        end
        
        RodShopSection:NewDropdown("Filter Category", "Filter rods by category", categories, function(selectedCategory)
            if selectedCategory == "All" then
                -- Reset dropdown to show all rods (this requires rebuilding the dropdown)
                debugPrint("🔍 [Rod Shop] Showing all purchasable rods")
            else
                local categoryRods = getRodsByCategory(selectedCategory)
                -- Filter to only purchasable rods
                local purchasableInCategory = {}
                for _, rodName in pairs(categoryRods) do
                    local rodInfo = getRodInfo(rodName)
                    if rodInfo.price > 0 then
                        table.insert(purchasableInCategory, rodName)
                    end
                end
                debugPrint("🔍 [Rod Shop] Filtered to " .. selectedCategory .. ": " .. #purchasableInCategory .. " rods")
            end
        end)
        
        -- Quantity selector
        RodShopSection:NewSlider("Purchase Quantity", "Number of rods to buy", 1, 10, 1, function(quantity)
            rodPurchaseFlags.purchaseQuantity = quantity
            if rodPurchaseFlags.selectedRod ~= "" then
                local rodInfo = getRodInfo(rodPurchaseFlags.selectedRod)
                local totalCost = rodInfo.price * quantity
                debugPrint("💰 [Rod Shop] " .. quantity .. "x " .. rodPurchaseFlags.selectedRod .. " = " .. formatPrice(totalCost))
            end
        end)
        
        -- Purchase button
        RodShopSection:NewButton("💳 Purchase Rod", "Buy the selected rod", function()
            if rodPurchaseFlags.selectedRod == "" then
                debugPrint("⚠️ [Rod Shop] Please select a rod first!")
                return
            end
            
            if rodPurchaseFlags.purchaseInProgress then
                debugPrint("⏳ [Rod Shop] Purchase already in progress...")
                return
            end
            
            local rodInfo = getRodInfo(rodPurchaseFlags.selectedRod)
            local totalCost = rodInfo.price * rodPurchaseFlags.purchaseQuantity
            
            debugPrint("🛒 [Rod Shop] Purchasing " .. rodPurchaseFlags.purchaseQuantity .. "x " .. rodPurchaseFlags.selectedRod)
            debugPrint("💰 [Rod Shop] Total cost: " .. formatPrice(totalCost))
            debugPrint("📍 [Rod Shop] Will teleport to: " .. rodInfo.category .. " shop")
            
            task.spawn(function()
                purchaseRod(rodPurchaseFlags.selectedRod, rodPurchaseFlags.purchaseQuantity)
            end)
        end)
        
        -- Rod information display
        RodShopSection:NewButton("ℹ️ Rod Info", "Display selected rod information", function()
            if rodPurchaseFlags.selectedRod == "" then
                debugPrint("⚠️ [Rod Shop] Please select a rod first!")
                return
            end
            ...
            local rodInfo = getRodInfo(rodPurchaseFlags.selectedRod)
            debugPrint("📋 [Rod Info] ===== " .. rodPurchaseFlags.selectedRod .. " =====")
            debugPrint("💰 [Rod Info] Price: " .. formatPrice(rodInfo.price))
            debugPrint("🏷️ [Rod Info] Category: " .. rodInfo.category)
            debugPrint("📝 [Rod Info] Description: " .. rodInfo.description)
            debugPrint("📍 [Rod Info] Shop Location: " .. tostring(rodInfo.location))
            debugPrint("💳 [Rod Info] Currency: " .. rodInfo.currency)
            
            if rodPurchaseFlags.purchaseQuantity > 1 then
                local totalCost = rodInfo.price * rodPurchaseFlags.purchaseQuantity
                debugPrint("🛒 [Rod Info] " .. rodPurchaseFlags.purchaseQuantity .. "x Cost: " .. formatPrice(totalCost))
            end
        end)
        
        -- Quick purchase presets
        local QuickBuySection = ShopTab:NewSection("⚡ Quick Buy Presets")
        
        QuickBuySection:NewButton("🥉 Buy All Basic Rods", "Purchase all basic tier rods", function()
            local basicRods = getRodsByCategory("Basic")
            local purchasableBasic = {}
            for _, rodName in pairs(basicRods) do
                local rodInfo = getRodInfo(rodName)
                if rodInfo.price > 0 then
                    table.insert(purchasableBasic, rodName)
                end
            end
            
            if #purchasableBasic > 0 then
                rodPurchaseFlags.autoPurchaseEnabled = true
                debugPrint("🛒 [Quick Buy] Starting purchase of " .. #purchasableBasic .. " basic rods...")
                task.spawn(function()
                    bulkPurchaseRods(purchasableBasic, 1)
                    rodPurchaseFlags.autoPurchaseEnabled = false
                end)
            else
                debugPrint("❌ [Quick Buy] No purchasable basic rods found!")
            end
        end)
        
        QuickBuySection:NewButton("🥈 Buy All Standard Rods", "Purchase all standard tier rods", function()
            local standardRods = getRodsByCategory("Standard")
            local purchasableStandard = {}
            for _, rodName in pairs(standardRods) do
                local rodInfo = getRodInfo(rodName)
                if rodInfo.price > 0 then
                    table.insert(purchasableStandard, rodName)
                end
            end
            
            if #purchasableStandard > 0 then
                rodPurchaseFlags.autoPurchaseEnabled = true
                debugPrint("🛒 [Quick Buy] Starting purchase of " .. #purchasableStandard .. " standard rods...")
                task.spawn(function()
                    bulkPurchaseRods(purchasableStandard, 1)
                    rodPurchaseFlags.autoPurchaseEnabled = false
                end)
            else
                debugPrint("❌ [Quick Buy] No purchasable standard rods found!")
            end
        end)
        
        QuickBuySection:NewButton("🥇 Buy All Premium Rods", "Purchase all premium tier rods", function()
            local premiumRods = getRodsByCategory("Premium")
            local purchasablePremium = {}
            for _, rodName in pairs(premiumRods) do
                local rodInfo = getRodInfo(rodName)
                if rodInfo.price > 0 then
                    table.insert(purchasablePremium, rodName)
                end
            end
            
            if #purchasablePremium > 0 then
                rodPurchaseFlags.autoPurchaseEnabled = true
                debugPrint("🛒 [Quick Buy] Starting purchase of " .. #purchasablePremium .. " premium rods...")
                task.spawn(function()
                    bulkPurchaseRods(purchasablePremium, 1)
                    rodPurchaseFlags.autoPurchaseEnabled = false
                end)
            else
                debugPrint("❌ [Quick Buy] No purchasable premium rods found!")
            end
        end)
        
        QuickBuySection:NewButton("💎 Buy ALL Purchasable Rods", "Purchase every buyable rod in the game", function()
            local allPurchasable = getAllPurchasableRods()
            
            if #allPurchasable > 0 then
                rodPurchaseFlags.autoPurchaseEnabled = true
                debugPrint("🛒 [Quick Buy] Starting MEGA purchase of ALL " .. #allPurchasable .. " purchasable rods!")
                debugPrint("⚠️ [Quick Buy] This will take several minutes and cost a lot of C$!")
                task.spawn(function()
                    bulkPurchaseRods(allPurchasable, 1)
                    rodPurchaseFlags.autoPurchaseEnabled = false
                end)
            else
                debugPrint("❌ [Quick Buy] No purchasable rods found!")
            end
        end)
        
        QuickBuySection:NewButton("⏹️ Stop Auto Purchase", "Cancel ongoing bulk purchases", function()
            rodPurchaseFlags.autoPurchaseEnabled = false
            debugPrint("🛑 [Quick Buy] Auto purchase cancelled by user")
        end)
        
        -- Statistics section
        local StatsSection = ShopTab:NewSection("📊 Purchase Statistics")
        
        StatsSection:NewButton("📈 Show Rod Statistics", "Display rod database statistics", function()
            local stats = {
                total = 0,
                purchasable = 0,
                quest = 0,
            ...
            for rodName, rodData in pairs(rodDatabase) do
                ...
                if rodData.price > 0 then
                    stats.purchasable = stats.purchasable + 1
                else
                    if rodData.category == "Quest" then
                        stats.quest = stats.quest + 1
                    elseif rodData.category == "Event" then
                        stats.event = stats.event + 1
                    end
                end
                
                stats.categories[rodData.category] = (stats.categories[rodData.category] or 0) + 1
            end
            
            debugPrint("📊 [Rod Statistics] ===== DATABASE OVERVIEW =====")
            debugPrint("📋 [Rod Statistics] Total Rods: " .. stats.total)
            debugPrint("💰 [Rod Statistics] Purchasable: " .. stats.purchasable)
            debugPrint("🎯 [Rod Statistics] Quest Only: " .. stats.quest)
            debugPrint("🎉 [Rod Statistics] Event Only: " .. stats.event)
            debugPrint("📊 [Rod Statistics] Categories:")
            
            for category, count in pairs(stats.categories) do
                debugPrint("  • " .. category .. ": " .. count .. " rods")
            end
            
            debugPrint("🛒 [Rod Statistics] Total Purchased This Session: " .. rodPurchaseFlags.totalPurchased)
        end)
        
        StatsSection:NewButton("💰 Calculate Total Cost", "Show cost to buy all purchasable rods", function()
            local totalCost = 0
            local expensiveRods = {}
            
            for rodName, rodData in pairs(rodDatabase) do
                if rodData.price > 0 then
                    totalCost = totalCost + rodData.price
                    
                    if rodData.price >= 5000 then
                        table.insert(expensiveRods, {name = rodName, price = rodData.price})
                    end
                end
            end
            
            -- Sort expensive rods by price (highest first)
            table.sort(expensiveRods, function(a, b) return a.price > b.price end)
            
            debugPrint("💰 [Cost Calculator] ===== PURCHASE ANALYSIS =====")
            debugPrint("🏦 [Cost Calculator] Total Cost for All Purchasable Rods: " .. formatPrice(totalCost))
            debugPrint("💎 [Cost Calculator] Most Expensive Rods:")
            
            for i = 1, math.min(5, #expensiveRods) do
                local rod = expensiveRods[i]
                debugPrint("  " .. i .. ". " .. rod.name .. " - " .. formatPrice(rod.price))
            end
            
            local purchasableCount = #getAllPurchasableRods()
            if purchasableCount > 0 then
                local avgCost = totalCost / purchasableCount
                debugPrint("📊 [Cost Calculator] Average Rod Price: " .. formatPrice(avgCost))
            end
        end)
        
        -- Advanced Features Section
        local AdvancedSection = ShopTab:NewSection("🔧 Advanced Rod Features")
        
        AdvancedSection:NewButton("🔍 Search Rods", "Search for rods by name", function()
            debugPrint("🔍 [Rod Search] Enter search term in chat (e.g., 'mythical', 'carbon', 'king')")
            debugPrint("💡 [Rod Search] Example searches: 'rod', 'aurora', 'kraken', 'lucky'")
            
            -- Simple search demo with common terms
            local commonSearches = {"mythical", "carbon", "lucky", "king", "aurora"}
            for _, term in pairs(commonSearches) do
                local results = searchRodsByName(term)
                if #results > 0 then
                    debugPrint("🔍 [Rod Search] '" .. term .. "' found " .. #results .. " matches:")
                    for i = 1, math.min(3, #results) do
                        local result = results[i]
                        debugPrint("  • " .. result.name .. " - " .. formatPrice(result.data.price))
                    end
                end
            end
        end)
        
        AdvancedSection:NewButton("🎯 Get Recommendations", "Get rod recommendations based on budget", function()
            local playerCurrency = getPlayerCurrency()
            local budget = playerCurrency > 0 and playerCurrency or 5000 -- Default budget if can't detect
            
            debugPrint("🎯 [Recommendations] Analyzing rods for budget: " .. formatPrice(budget))
            
            local recommendations = getRecommendedRods(1, budget)
            
            debugPrint("🎯 [Recommendations] ===== TOP RECOMMENDATIONS =====")
            for i = 1, math.min(5, #recommendations) do
                local rec = recommendations[i]
                debugPrint("  " .. i .. ". " .. rec.name .. " - " .. formatPrice(rec.data.price) .. 
                          " (" .. rec.data.category .. ") Score: " .. rec.score)
                debugPrint("     📝 " .. rec.data.description)
            end
        end)
        
        AdvancedSection:NewButton("💳 Check Affordable Rods", "Show rods you can afford right now", function()
            local playerCurrency = getPlayerCurrency()
            
            if playerCurrency <= 0 then
                debugPrint("💳 [Affordable] Could not detect your current currency")
                debugPrint("💡 [Affordable] Showing rods under 1000 C$ instead:")
                playerCurrency = 1000
            else
                debugPrint("💰 [Affordable] Your current currency: " .. formatPrice(playerCurrency))
            end
            
            local affordableRods = {}
            for rodName, rodData in pairs(rodDatabase) do
                if rodData.price > 0 and rodData.price <= playerCurrency then
                    table.insert(affordableRods, {name = rodName, data = rodData})
                end
            end
            
            -- Sort by price (cheapest first)
            table.sort(affordableRods, function(a, b) return a.data.price < b.data.price end)
            
            debugPrint("💳 [Affordable] You can afford " .. #affordableRods .. " rods:")
            for i = 1, math.min(10, #affordableRods) do
                local rod = affordableRods[i]
                debugPrint("  • " .. rod.name .. " - " .. formatPrice(rod.data.price) .. 
                          " (" .. rod.data.category .. ")")
            end
            
            if #affordableRods > 10 then
                debugPrint("  ... and " .. (#affordableRods - 10) .. " more!")
            end
        end)
        
        AdvancedSection:NewButton("🏆 Premium Rod Showcase", "Display all premium and legendary rods", function()
            debugPrint("🏆 [Showcase] ===== PREMIUM & LEGENDARY RODS =====")
            
            local premiumCategories = {"Premium", "Quest", "Event", "Special"}
            
            for _, category in pairs(premiumCategories) do
                local categoryRods = getRodsByCategory(category)
                if #categoryRods > 0 then
                    debugPrint("🏷️ [Showcase] " .. category .. " Rods:")
                    for _, rodName in pairs(categoryRods) do
                        local rodInfo = getRodInfo(rodName)
                        local priceText = rodInfo.price > 0 and formatPrice(rodInfo.price) or "Quest/Event Only"
                        debugPrint("  ⭐ " .. rodName .. " - " .. priceText)
                        debugPrint("     📝 " .. rodInfo.description)
                    end
                end
            end
        end)
        ...
        -- Utility Section
        local UtilitySection = ShopTab:NewSection("🛠️ Rod Shop Utilities")
        
        UtilitySection:NewButton("📊 Purchase Validation", "Check if you can purchase selected rod", function()
            if rodPurchaseFlags.selectedRod == "" then
                debugPrint("⚠️ [Validation] Please select a rod first!")
                return
            end
            
            local validation = validatePurchase(rodPurchaseFlags.selectedRod, rodPurchaseFlags.purchaseQuantity)
            
            debugPrint("🔍 [Validation] ===== PURCHASE VALIDATION =====")
            debugPrint("🎣 [Validation] Rod: " .. rodPurchaseFlags.selectedRod)
            debugPrint("🔢 [Validation] Quantity: " .. rodPurchaseFlags.purchaseQuantity)
            
            if validation.valid then
                debugPrint("✅ [Validation] Purchase is VALID!")
            else
                debugPrint("❌ [Validation] Purchase is INVALID!")
            end
            ...
            if #validation.warnings > 0 then
                debugPrint("⚠️ [Validation] Warnings:")
                for _, warning in pairs(validation.warnings) do
                    debugPrint("  • " .. warning)
                end
            end
            ...
            local rodInfo = getRodInfo(rodPurchaseFlags.selectedRod)
            local totalCost = rodInfo.price * rodPurchaseFlags.purchaseQuantity
            debugPrint("💰 [Validation] Total Cost: " .. formatPrice(totalCost))
        end)
        
        UtilitySection:NewButton("🔄 Refresh Rod Database", "Reload and verify rod database", function()
            debugPrint("🔄 [Refresh] Refreshing rod database...")
            
            local totalRods = 0
            local purchasableRods = 0
            local questRods = 0
            
            for rodName, rodData in pairs(rodDatabase) do
                totalRods = totalRods + 1
                if rodData.price > 0 then
                    purchasableRods = purchasableRods + 1
                else
                    questRods = questRods + 1
                end
            end
            
            debugPrint("✅ [Refresh] Database refreshed successfully!")
            debugPrint("📊 [Refresh] Total: " .. totalRods .. " | Purchasable: " .. purchasableRods .. " | Quest/Event: " .. questRods)
            debugPrint("🏷️ [Refresh] Categories: " .. #getRodCategories())
        end)
        
        UtilitySection:NewButton("💾 Export Rod List", "Export all rod names to chat", function()
            debugPrint("💾 [Export] ===== ALL RODS IN DATABASE =====")
            debugPrint("📝 [Export] Exporting " .. #getAllPurchasableRods() .. " purchasable rods:")
            
            local categories = getRodCategories()
            for _, category in pairs(categories) do
                ...
                if #purchasableInCategory > 0 then
                    debugPrint("🏷️ [Export] " .. category .. " (" .. #purchasableInCategory .. "):")
                    for _, rodName in pairs(purchasableInCategory) do
                        local rodInfo = getRodInfo(rodName)
                        debugPrint("  • " .. rodName .. " - " .. formatPrice(rodInfo.price))
                    end
                end
            end
        end)
        
        UtilitySection:NewLabel("🔧 Features: Search, Recommendations, Validation, Bulk Purchase")
        UtilitySection:NewLabel("💡 Tip: Enable Debug Mode in Settings to see detailed output!")
        UtilitySection:NewLabel("🎯 Total Session Purchases: " .. rodPurchaseFlags.totalPurchased)
...
if Window and Window.NewTab then
    pcall(function()
        ...
        PlayerTab = Window:NewTab("👥 Player Teleport")
...
if PlayerTab then
    local PlayerMainSection = PlayerTab:NewSection("👥 Player Selection")
    
    -- Player list dropdown (refreshes automatically)
    local playerDropdown = nil
    playerDropdown = PlayerMainSection:NewDropdown("Select Player", "Choose a player to teleport to", getPlayerListForUI(), function(currentOption)
        local playerName = extractPlayerNameFromUIText(currentOption)
        if playerName then
            selectedPlayerName = playerName
            debugPrint("👤 [Player Teleport] Selected player: " .. playerName)
            
            -- Update follow target if following is enabled
            if flags['followplayer'] then
                startFollowPlayer(playerName)
            end
        end
    end)
    ...
    spawn(function()
        while true do
            wait(playerRefreshRate)
            if PlayerTab and playerDropdown then
                pcall(function()
                    local newList = getPlayerListForUI()
    ...
    local PlayerActionSection = PlayerTab:NewSection("🚀 Teleport Actions")
    
    PlayerActionSection:NewButton("📍 Teleport to Selected Player", "Instantly teleport to the selected player", function()
        if selectedPlayerName == "" then
            debugPrint("❌ [Player Teleport] Please select a player first!")
            return
        end
    ...
    PlayerActionSection:NewButton("🔄 Refresh Player List", "Manually refresh the player list", function()
        lastPlayerRefresh = 0 -- Force refresh
        local newList = getPlayerListForUI()
        if playerDropdown then
            playerDropdown:Refresh(newList)
        end
        debugPrint("🔄 [Player Teleport] Player list refreshed! Found " .. (#newList - 1) .. " players")
    end)
    
    PlayerActionSection:NewButton("📊 Show Player Info", "Display information about all players", function()
        local players = getAllPlayers()
        
        debugPrint("👥 [Player Info] Current Server Players (" .. #players .. " players):")
        debugPrint("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        
        for i, playerData in pairs(players) do
            local info = "  " .. i .. ". " .. playerData.name
            
            if playerData.displayName ~= playerData.name then
                info = info .. " (" .. playerData.displayName .. ")"
            end
            
            info = info .. " - " .. playerData.distance .. " studs away"
    ...
    local PlayerFollowSection = PlayerTab:NewSection("🏃 Follow System")
    
    PlayerFollowSection:NewToggle("Follow Selected Player", "Automatically follow the selected player", function(state)
        flags['followplayer'] = state
        
        if state then
            if selectedPlayerName ~= "" then
                startFollowPlayer(selectedPlayerName)
            else
                debugPrint("❌ [Player Follow] Please select a player first!")
                flags['followplayer'] = false
            end
        else
            stopFollowPlayer()
    ...
    PlayerFollowSection:NewSlider("Follow Distance", "Distance to maintain when following (studs)", 5, 50, function(val)
        -- This would be used in follow logic if we want variable follow distance
        debugPrint("🏃 [Player Follow] Follow distance set to: " .. val .. " studs")
    end)
    
    local PlayerSafetySection = PlayerTab:NewSection("🛡️ Safety Settings")
    
    PlayerSafetySection:NewToggle("Enable Teleport Safety", "Enable safety checks before teleporting", function(state)
        flags['teleportsafety'] = state
        debugPrint("🛡️ [Player Safety] Teleport safety: " .. (state and "Enabled" or "Disabled"))
    end)
    
    PlayerSafetySection:NewSlider("Max Teleport Distance", "Maximum allowed teleport distance", 100, 50000, function(val)
        flags['maxtelportdistance'] = val
        debugPrint("🛡️ [Player Safety] Max teleport distance set to: " .. val .. " studs")
    end)
    
    PlayerSafetySection:NewButton("⚠️ Emergency Stop Follow", "Immediately stop following any player", function()
        stopFollowPlayer()
        debugPrint("🛑 [Player Safety] Emergency stop activated - Following disabled")
    end)
    
    local PlayerUtilSection = PlayerTab:NewSection("🔧 Player Utilities")
    
    PlayerUtilSection:NewTextBox("Search Player by Name", "Type player name to search", function(txt)
        if txt and txt ~= "" then
            local matchingPlayers = searchPlayersByName(txt)
            
            debugPrint("🔍 [Player Search] Search results for '" .. txt .. "':")
            if #matchingPlayers > 0 then
                for i, playerData in pairs(matchingPlayers) do
                    local info = "  " .. i .. ". " .. playerData.name
                    if playerData.displayName ~= playerData.name then
                        info = info .. " (" .. playerData.displayName .. ")"
                    end
                    info = info .. " - " .. playerData.distance .. " studs"
                    debugPrint(info)
                end
                
                -- Auto-select first match
                if #matchingPlayers == 1 then
                    selectedPlayerName = matchingPlayers[1].name
                    debugPrint("✅ [Player Search] Auto-selected: " .. selectedPlayerName)
                end
            else
                debugPrint("  No players found matching '" .. txt .. "'")
            end
        end
    end)
    
    PlayerUtilSection:NewButton("🎯 Teleport to Closest Player", "Teleport to the closest player", function()
        local players = getAllPlayers()
        
        if #players > 0 then
            local closestPlayer = players[1] -- Already sorted by distance
            teleportToPlayer(closestPlayer)
        else
            debugPrint("❌ [Player Teleport] No players found to teleport to")
        end
    end)
    
    PlayerUtilSection:NewButton("📋 Copy Player Names", "Copy all player names to console", function()
        local players = getAllPlayers()
        local nameList = {}
        
        for _, playerData in pairs(players) do
            table.insert(nameList, playerData.name)
        end
        
        local namesString = table.concat(nameList, ", ")
        debugPrint("📋 [Player Names] Current players: " .. namesString)
        debugPrint("💡 [Player Names] Total: " .. #players .. " players (excluding you)")
    end)
end
...
-- 🎣 AUTO FISHING LOOP SECTION - COMPLETE AUTOMATION
local LoopSection = AutoTab:NewSection("🎣 Auto Fishing Loop (Complete Automation)")

LoopSection:NewToggle("🔄 Auto Fishing Loop", "Complete automation: Auto Equip → Cast → Shake → Reel → Repeat", function(state)
    flags['autofishingloop'] = state
    if state then
        debugPrint("🔄 [Auto Fishing Loop] MASTER TOGGLE ACTIVATED!")
        debugPrint("🎣 [Auto Fishing Loop] Complete automation cycle started!")
        debugPrint("⚡ [Auto Fishing Loop] Will auto-equip rod and repeat fishing cycle!")
    else
        debugPrint("🔄 [Auto Fishing Loop] MASTER TOGGLE DEACTIVATED!")
    end
end)

LoopSection:NewToggle("🎣 Auto Equip Rod", "Automatically equip best fishing rod from inventory", function(state)
    flags['autoequiprod'] = state
    if state then
        debugPrint("🎣 [Auto Equip Rod] Activated - Will auto-equip best rod!")
    else
        debugPrint("🎣 [Auto Equip Rod] Deactivated")
    end
end)

local availableRods = {} -- Initialize empty, will be populated later
LoopSection:NewDropdown("Select Preferred Rod", "Choose specific rod to auto-equip (empty = auto-select best)", availableRods, function(rodName)
    selectedRodName = rodName or ""
    if selectedRodName ~= "" then
        debugPrint("🎯 [Rod Selection] Preferred rod set to: " .. selectedRodName)
    else
        debugPrint("🎯 [Rod Selection] Auto-select best rod enabled")
    end
end)

LoopSection:NewSlider("⏱️ Equip Delay", "Delay after equipping rod (seconds)", 0.1, 5.0, 0.5, function(value)
    flags['equipdelay'] = value
    debugPrint("⏱️ [Equip Delay] Set to " .. value .. " seconds")
end)

LoopSection:NewToggle("🔄 Auto Restart", "Restart fishing cycle if no progress detected", function(state)
    flags['autorestart'] = state
    if state then
        debugPrint("🔄 [Auto Restart] Enabled - will restart if stuck!")
    else
        debugPrint("🔄 [Auto Restart] Disabled")
    end
end)

LoopSection:NewSlider("⏰ Restart Timeout", "Restart if no progress for X seconds", 5, 30, 10, function(value)
    flags['restarttimeout'] = value
    debugPrint("⏰ [Restart Timeout] Set to " .. value .. " seconds")
end)

LoopSection:NewTextBox("🎯 Specific Rod Name", "Enter exact name of rod to equip (case sensitive)", function(text)
    selectedRodName = text or ""
    if selectedRodName ~= "" then
        debugPrint("🎯 [Specific Rod] Set to: " .. selectedRodName)
    else
        debugPrint("🎯 [Specific Rod] Cleared - will use auto-selection")
    end
end)

LoopSection:NewButton("🔄 Refresh Rod List", "Update available rod list from inventory", function()
    local currentRods = getRodsFromInventory()
    debugPrint("🔄 [Rod Refresh] Found " .. #currentRods .. " rods in inventory:")
    for i, rod in pairs(currentRods) do
        debugPrint("  " .. i .. ". " .. rod)
    end
    -- Update the availableRods table
    availableRods = currentRods
end)

LoopSection:NewLabel("Auto Fishing Loop combines Auto Equip + Cast + Shake + Reel for complete automation!")
LoopSection:NewLabel("🔄 Complete Fishing Cycle: Equip Rod → Cast → Shake → Reel → Repeat")
LoopSection:NewLabel("⚡ Best used with Super Instant Reel for maximum speed!")
LoopSection:NewLabel("🎯 Enable Debug Mode in Settings to see real-time status")

-- 🕐 SERVER AGE DETECTOR SECTION
local ServerSection = AutoTab:NewSection("🕐 Server Age Detector & Old Server Finder")

ServerSection:NewToggle("🕐 Server Age Checker", "Monitor current server age and metrics", function(state)
    flags['serveragechecker'] = state
    if state then
        debugPrint("🕐 [Server Age] Server monitoring activated!")
        displayServerInfo() -- Show initial info
    else
        debugPrint("🕐 [Server Age] Server monitoring disabled")
    end
end)

ServerSection:NewToggle("🚀 Auto Join Old Server", "Automatically join better old servers", function(state)
    flags['autojoinoldserver'] = state
    if state then
        debugPrint("🚀 [Auto Server Hop] Auto server hopping enabled!")
        debugPrint("⚠️ [Auto Server Hop] Will automatically switch to better servers!")
    else
        debugPrint("🚀 [Auto Server Hop] Auto server hopping disabled")
    end
end)

ServerSection:NewSlider("👥 Max Player Count", "Maximum players in target server", 5, 20, 12, function(value)
    flags['maxplayercount'] = value
    debugPrint("👥 [Server Settings] Max players set to: " .. value)
end)

ServerSection:NewSlider("⏰ Min Server Age", "Minimum server age in seconds", 60, 1800, 300, function(value)
    flags['minserverage'] = value
    debugPrint("⏰ [Server Settings] Min server age set to: " .. value .. "s (" .. math.floor(value/60) .. "m)")
end)

ServerSection:NewButton("📊 Show Server Info", "Display detailed server information", function()
    displayServerInfo()
end)

ServerSection:NewButton("🔍 Find Old Servers", "Search for and join old servers manually", function()
    debugPrint("🔍 [Manual Server Hop] Searching for old servers...")
    if joinOldServer() then
        debugPrint("✅ [Manual Server Hop] Joining better server...")
    else
        debugPrint("❌ [Manual Server Hop] No better servers found")
    end
end)

ServerSection:NewButton("🎯 Check Current Server", "Analyze if current server is optimal", function()
    local metrics = getServerMetrics()
    local isGood, score, total = isGoodOldServer(metrics)
    
    debugPrint("🎯 [Server Analysis] Current Server Quality Check:")
    debugPrint("  Result: " .. (isGood and "✅ GOOD OLD SERVER" or "❌ Not optimal"))
    debugPrint("  Score: " .. score .. "/" .. total .. " criteria met")
    debugPrint("  Recommendation: " .. (isGood and "Stay in this server" or "Consider server hopping"))
end)

ServerSection:NewButton("📋 Server History", "Show recent server history", function()
    displayServerHistory()
end)

ServerSection:NewButton("📈 Session Stats", "Show server hopping session statistics", function()
    displaySessionStats()
end)

ServerSection:NewButton("🔄 Reset Session", "Reset session statistics", function()
    currentSessionStats = {
        serversVisited = 0,
        bestServerScore = 0,
        totalFishCaught = 0,
    ...
    serverHistory = {}
    debugPrint("🔄 [Session Reset] Statistics reset!")
end)

ServerSection:NewLabel("🕐 Old servers usually have: Less players, more fish, stable events")
ServerSection:NewLabel("🎯 Best servers: 5-12 players, 5+ minutes old, active events")
ServerSection:NewLabel("⚠️ Auto hop will search for better servers automatically")

LoopSection:NewButton("⛔ Emergency Stop", "Immediately stop all auto fishing activities", function()
    ...
    flags['superinstantreel'] = false
    debugPrint("⛔ [Emergency Stop] ALL AUTO FISHING ACTIVITIES STOPPED!")
end)

LoopSection:NewButton("📊 Show Status", "Display current auto fishing loop status", function()
    debugPrint("📊 [Status Check] Auto Fishing Loop Status:")
    debugPrint("  🔄 Master Toggle: " .. tostring(flags['autofishingloop']))
    debugPrint("  🎣 Auto Equip Rod: " .. tostring(flags['autoequiprod']))
    debugPrint("  ⏱️ Equip Delay: " .. tostring(flags['equipdelay']) .. "s")
    debugPrint("  🎯 Selected Rod: " .. (selectedRodName ~= "" and selectedRodName or "Auto-select best"))
    debugPrint("  🎣 Currently Equipped: " .. (hasRodEquipped() and FindRod().Name or "None"))
    local availableCount = #getRodsFromInventory()
    debugPrint("  📦 Available Rods: " .. availableCount .. " in inventory")
end)
...
-- NEW: No Reel Movement Toggle (STANDALONE)
ReelSection:NewToggle("No Reel Movement", "🚫 Remove character movement during reel minigame (works independently)", function(state)
    flags['noreelmovement'] = state
    if state then
        debugPrint("🚫 [No Reel Movement] Character movement during reel disabled!")
        debugPrint("🎯 [Smooth Reel] Reel minigame without character jerking!")
    else
        debugPrint("🎬 [Reel Movement] Normal character movement during reel restored")
    end
end)

-- NEW: Instant Reel Completion Toggle
ReelSection:NewToggle("Instant Reel Complete", "⚡ Skip reel minigame entirely - instant completion", function(state)
    flags['instantreelcomplete'] = state
    if state then
        debugPrint("⚡ [Instant Reel] Reel minigame will be skipped instantly!")
        debugPrint("🚀 [Zero Delay] No reel animation or waiting time!")
    else
        debugPrint("🎮 [Normal Reel] Standard reel minigame restored")
    end
end)
...
-- Reel Animation Control Info
ReelSection:NewLabel("🎬 Reel Animation Options:")
ReelSection:NewLabel("• Disable Reel Animations: Works with Super Instant Reel")
ReelSection:NewLabel("• No Reel Movement: Remove character jerking (independent)")
ReelSection:NewLabel("• Instant Reel Complete: Skip minigame entirely")
ReelSection:NewLabel("💡 Choose based on your preference!")

-- 🏆 AUTO APPRAISE SECTION
local AppraiseSection = AutoTab:NewSection("🏆 Auto Appraise")
AppraiseSection:NewToggle("Auto Appraise", "🏆 Automatically appraise fish when inventory is full", function(state)
    flags['autoappraise'] = state
    if state then
        debugPrint("🏆 [Auto Appraise] ACTIVATED - Fish will be appraised automatically!")
        debugPrint("💰 [Appraise System] Using intelligent NPC detection & AppraiseAnywhere backup")
    else
        debugPrint("⏸️ [Auto Appraise] Deactivated")
    end
end)

AppraiseSection:NewDropdown("Appraise Mode", "🎯 Select appraise method", {"Smart (NPC + Backup)", "NPC Dialog Only", "AppraiseAnywhere Only"}, function(currentOption)
    appraiseMode = currentOption
    debugPrint("🎯 [Appraise Mode] Set to: " .. currentOption)
end)

AppraiseSection:NewSlider("Check Interval", "⏱️ How often to check for appraisal (seconds)", 1, 10, function(value)
    appraiseCheckInterval = value
    debugPrint("⏱️ [Auto Appraise] Check interval: " .. value .. " seconds")
end)

AppraiseSection:NewSlider("NPC Search Distance", "🔍 Maximum distance to search for NPC appraisers", 50, 500, function(value)
    npcSearchDistance = value
    debugPrint("🔍 [Auto Appraise] NPC search distance: " .. value .. " studs")
end)

AppraiseSection:NewLabel("💡 Smart Mode: Tries NPC first, uses AppraiseAnywhere as backup")
AppraiseSection:NewLabel("🤖 NPC Mode: Only uses dialog with NPC appraisers")  
AppraiseSection:NewLabel("⚡ AppraiseAnywhere: Only uses instant remote appraise")
...
RunService.Heartbeat:Connect(function()
    -- 🕐 SERVER AGE DETECTOR & AUTO SERVER HOP
    if flags['serveragechecker'] then
        local currentTime = tick()
        ...
        if currentTime % 10 < 0.1 then
            ...
            -- Auto join old server if current server is bad
            if flags['autojoinoldserver'] then
                local isGood, score, total = isGoodOldServer(metrics)
                
                if not isGood and currentTime - lastServerHop > 60 then -- Check every minute
                    debugPrint("⚠️ [Auto Server Hop] Current server not optimal (score: " .. score .. "/" .. total .. ")")
                    debugPrint("🔍 [Auto Server Hop] Searching for better server...")
                    
                    -- Try to find and join better server
                    joinOldServer()
                end
            end
        end
        
        -- Display server info every 30 seconds if debug enabled
        if flags['debugmode'] and currentTime % 30 < 0.1 then
            displayServerInfo()
        end
    end

    -- 🎣 AUTO FISHING LOOP SYSTEM - COMPLETE AUTOMATION CYCLE
    if flags['autofishingloop'] then
        local currentTime = tick()
        
        -- Step 1: Auto Equip Rod (if needed)
        if flags['autoequiprod'] and not hasRodEquipped() then
            if autoEquipBestRod() then
                task.wait(flags['equipdelay'] or 0.5) -- Wait after equipping
                lastProgressTime = currentTime -- Reset progress timer
            end
        end
        
        -- Ensure we have a rod before proceeding with fishing cycle
        local currentRod = FindRod()
        if currentRod then
            -- Check for progress (lure value changing or successful catches)
            local lureValue = currentRod.values.lure and currentRod.values.lure.Value or 0
            if lureValue > 50 or lureValue < 5 then -- Progress detected (casting or catching)
                lastProgressTime = currentTime
            end
            
            -- Step 2: Enable auto features for complete fishing cycle
            if flags['autocastloop'] then
                flags['autocast'] = true -- Enable auto cast
            end
            if flags['autoshakeloop'] then 
                flags['autoshake'] = true -- Enable auto shake
            end
            if flags['autoreelloop'] then
                flags['autoreel'] = true -- Enable auto reel
                flags['superinstantreel'] = true -- Use super instant reel for speed
            end
            
            -- Step 3: Auto restart if stuck (no progress for too long)
            if flags['autorestart'] and (currentTime - lastProgressTime) > (flags['restarttimeout'] or 10) then
                debugPrint("🔄 [Auto Restart] No progress detected, restarting fishing cycle...")
                -- Force re-equip rod to restart cycle
                pcall(function()
                    currentRod.Parent = lp.Backpack -- Unequip current rod
                end)
                lastProgressTime = currentTime
            end
            
            -- Debug output for active loop
            if flags['debugmode'] and currentTime % 5 < 0.1 then -- Print every 5 seconds
                debugPrint("🔄 [Auto Fishing Loop] Active - Rod: " .. currentRod.Name .. 
                          " | Lure: " .. math.floor(lureValue) .. "% | Time since progress: " .. 
                          math.floor(currentTime - lastProgressTime) .. "s")
            end
        else
            -- No rod available, show warning and try to equip
            if flags['autoequiprod'] then
                lastProgressTime = currentTime -- Reset timer when trying to equip
            end
            
            if flags['debugmode'] and currentTime % 10 < 0.1 then -- Print every 10 seconds
                debugPrint("❌ [Auto Fishing Loop] No rod equipped and none available in inventory!")
            end
        end
    else
        -- Auto fishing loop is disabled, don't override individual settings
    end
    ...
    -- 🏆 AUTO APPRAISE SYSTEM - Automatic Fish Appraisal
    if flags['autoappraise'] then
        if tick() - lastAppraiseCheck >= appraiseCheckInterval then
            lastAppraiseCheck = tick()
            task.spawn(function()
                pcall(function()
                    autoAppraise()
                end)
    ...
    -- 🚫 NO REEL MOVEMENT SYSTEM - Remove character jerking during reel
    if flags['noreelmovement'] then
        pcall(function()
            local character = lp.Character
            if character and character:FindFirstChild("Humanoid") then
                local humanoid = character.Humanoid
                
                -- Check if reel GUI is active
                local reelUI = lp.PlayerGui:FindFirstChild("reel")
                if reelUI then
                    -- Freeze character position during reel
                    if character:FindFirstChild("HumanoidRootPart") then
                        local hrp = character.HumanoidRootPart
                        if not hrp:GetAttribute("ReelFrozen") then
                            hrp:SetAttribute("ReelFrozen", true)
                            hrp:SetAttribute("OriginalAnchoredState", hrp.Anchored)
                            hrp.Anchored = true
                        end
                    end
                    
                    -- Stop all character animations during reel
                    for _, track in pairs(humanoid:GetPlayingAnimationTracks()) do
                        if track.Name:lower():find("reel") or 
                           track.Name:lower():find("fishing") or
                           track.Name:lower():find("pull") then
                            track:Stop()
                        end
                    end
                else
                    -- Restore character movement when reel is done
                    if character:FindFirstChild("HumanoidRootPart") then
                        local hrp = character.HumanoidRootPart
                        if hrp:GetAttribute("ReelFrozen") then
                            hrp:SetAttribute("ReelFrozen", nil)
                            local originalState = hrp:GetAttribute("OriginalAnchoredState")
                            if originalState ~= nil then
                                hrp.Anchored = originalState
                                hrp:SetAttribute("OriginalAnchoredState", nil)
                            end
                        end
                    end
                end
    ...
    -- ⚡ INSTANT REEL COMPLETE SYSTEM - Skip reel minigame entirely
    if flags['instantreelcomplete'] then
        pcall(function()
            local reelUI = lp.PlayerGui:FindFirstChild("reel")
            if reelUI then
                -- Instantly complete reel without any delay
                ReplicatedStorage.events.reelfinished:FireServer(100, true)
                
                -- Destroy reel UI immediately
                reelUI:Destroy()
                
                debugPrint("⚡ [Instant Reel Complete] Skipped reel minigame!")
            end
        end)
    end
                ...
                if flags['superinstantreel'] then -- Always block when super instant reel is on
                    ...
                    if animationId:find("fish") or animationId:find("reel") or animationId:find("cast") or 
                       ...
                       animationName:find("rod") or animationName:find("catch") or animationName:find("lift") then
                        debugPrint("🚫 [Super Instant] Animation blocked: " .. (animation.Name or "Unknown"))
                        return -- Don't play the animation
                    end
                end
                
                -- Block reel movement animations specifically (NEW)
                if flags['noreelmovement'] then
                    local animationId = tostring(animation.AnimationId):lower()
                    local animationName = tostring(animation.Name):lower()
                    
                    -- Block only reel-related movement animations
                    if animationId:find("reel") or animationId:find("pull") or animationId:find("yank") or
                       animationId:find("fishing_reel") or animationId:find("catch_pull") or
                       animationName:find("reel") or animationName:find("pull") or animationName:find("yank") then
                        debugPrint("🚫 [No Reel Movement] Reel animation blocked: " .. (animation.Name or "Unknown"))
                        return -- Block reel movement animations
                    end
                end
...
-- References to game world folders (Fixed based on actual game structure)
local FishingZonesFolder = nil -- No fishing zones folder found in workspace structure
local TpSpotsFolder = nil -- No spawns folder found in workspace structure  
local NpcFolder = workspace.world and workspace.world.npcs -- This exists based on data.txt
local ActiveFolder = workspace:FindFirstChild("active") -- Check if active folder exists

-- Function to scan and update fishing zones (Updated for actual game structure)
local function updateDynamicFishingZones()
    pcall(function()
        dynamicFishingZones = {}
        -- Since no fishing zones folder exists, we'll use known locations from the static teleport system
        if TeleportLocations and TeleportLocations['Zones'] then
            for zoneName, _ in pairs(TeleportLocations['Zones']) do
                if not table.find(dynamicFishingZones, zoneName) then
                    table.insert(dynamicFishingZones, zoneName)
                end
            end
        end
...
-- Function to scan and update teleport spots (Updated for actual game structure)
local function updateDynamicTeleportSpots()
    pcall(function()
        dynamicTeleportSpots = {}
        -- Use static teleport locations as base since no dynamic spots folder exists
        if TeleportLocations then
            for category, locations in pairs(TeleportLocations) do
                if category ~= 'All Locations' then -- Skip the combined category
                    for spotName, _ in pairs(locations) do
                        if not table.find(dynamicTeleportSpots, spotName) then
                            table.insert(dynamicTeleportSpots, spotName)
                        end
                    end
                end
            end
...
-- Function to scan active items (Updated - scan workspace for interactive objects)
local function updateDynamicActiveItems()
    pcall(function()
        dynamicActiveItems = {}
        -- Scan workspace for items with interaction capabilities
        local function scanFolder(folder, depth)
            if depth > 3 then return end -- Prevent infinite recursion
            
            for _, item in pairs(folder:GetChildren()) do
                pcall(function()
                    if item:FindFirstChild("PickupPrompt") or 
                       item:FindFirstChild("ProximityPrompt") or
                       item:FindFirstChild("ClickDetector") or
                       item.Name:lower():find("bottle") or
                       item.Name:lower():find("chest") or
                       item.Name:lower():find("treasure") then
                        
                        local itemName = item.Name
                        if not table.find(dynamicActiveItems, itemName) and itemName ~= "mirror Area" then
                            table.insert(dynamicActiveItems, itemName)
                        end
                    end
        ...
        -- Scan workspace for interactive items
...
debugPrint("🔍 [Teleport V2 Fixed] System structure analyzed:")
debugPrint("🎣 Fishing zones found: " .. #dynamicFishingZones .. " (from static teleport locations)")
debugPrint("🌍 Teleport spots found: " .. #dynamicTeleportSpots .. " (from all teleport categories)")
debugPrint("👤 NPCs found: " .. #dynamicNPCLocations .. " (from workspace.world.npcs)")
debugPrint("⭐ Interactive items found: " .. #dynamicActiveItems .. " (scanned from workspace)")
debugPrint("✅ [Fix Applied] Teleport V2 now uses correct game structure!")

-- Set up auto-updating listeners (Updated for actual structure)
if NpcFolder then
    NpcFolder.ChildAdded:Connect(function(child)
        if not table.find(dynamicNPCLocations, child.Name) and child.Name ~= "mirror Area" then
            ...
            table.sort(dynamicNPCLocations)
            debugPrint("👤 [Dynamic V2] New NPC added: " .. child.Name)
        end
    end)
end

-- Monitor workspace for new interactive items
workspace.ChildAdded:Connect(function(child)
    pcall(function()
        if child:FindFirstChild("PickupPrompt") or 
           child:FindFirstChild("ProximityPrompt") or
           child:FindFirstChild("ClickDetector") or
           child.Name:lower():find("bottle") or
           child.Name:lower():find("chest") or
           child.Name:lower():find("treasure") then
            
            if not table.find(dynamicActiveItems, child.Name) then
                table.insert(dynamicActiveItems, child.Name)
                table.sort(dynamicActiveItems)
                debugPrint("⭐ [Dynamic V2] New interactive item added: " .. child.Name)
            end
        end
    end)
end)
...
if TeleTabV2 then
    pcall(function()
        ...
        local dynamicFishingDropdown = DynamicFishingSection:NewDropdown("Dynamic Fishing Zones", "Teleport to automatically detected fishing zones", dynamicFishingZones, function(selectedZone)
            pcall(function()
                if TeleportLocations and TeleportLocations['Zones'] and TeleportLocations['Zones'][selectedZone] then
                    gethrp().CFrame = TeleportLocations['Zones'][selectedZone]
                    debugPrint("🎣 [Dynamic V2] Teleported to fishing zone: " .. selectedZone)
                else
                    debugPrint("❌ [Dynamic V2] Zone not found: " .. selectedZone)
                end
            end)
        end)
        ...
        local dynamicSpotsDropdown = DynamicSpotsSection:NewDropdown("Dynamic Teleport Spots", "Teleport to automatically detected locations", dynamicTeleportSpots, function(selectedSpot)
            pcall(function()
                -- Try to find in any teleport category
                local foundLocation = nil
                if TeleportLocations then
                    for category, locations in pairs(TeleportLocations) do
                        if locations[selectedSpot] then
                            foundLocation = locations[selectedSpot]
                            break
                        end
                    end
                end
                ...
                if foundLocation then
                    gethrp().CFrame = foundLocation
                    debugPrint("🌍 [Dynamic V2] Teleported to: " .. selectedSpot)
                else
                    debugPrint("❌ [Dynamic V2] Location not found: " .. selectedSpot)
                end
            end)
        end)
        ...
        local dynamicNPCDropdown = DynamicNPCSection:NewDropdown("Dynamic NPC Locations", "Teleport to automatically detected NPCs", dynamicNPCLocations, function(selectedNPC)
            pcall(function()
                -- First try static NPC locations
                if TeleportLocations and TeleportLocations['NPCs'] and TeleportLocations['NPCs'][selectedNPC] then
                    gethrp().CFrame = TeleportLocations['NPCs'][selectedNPC]
                    debugPrint("👤 [Dynamic V2] Teleported to NPC: " .. selectedNPC)
                -- Then try dynamic NPC scanning
                elseif NpcFolder then
                    local npc = NpcFolder:FindFirstChild(selectedNPC)
                    if npc and npc:FindFirstChild("HumanoidRootPart") then
                        gethrp().CFrame = npc.HumanoidRootPart.CFrame + Vector3.new(0, 0, 5)
                        debugPrint("👤 [Dynamic V2] Teleported to dynamic NPC: " .. selectedNPC)
                    elseif npc then
                        -- Try to get position from the model
                        local pos = npc:GetModelCFrame()
                        gethrp().CFrame = pos + Vector3.new(0, 0, 5)
                        debugPrint("👤 [Dynamic V2] Teleported to NPC model: " .. selectedNPC)
                    else
                        debugPrint("❌ [Dynamic V2] NPC not found: " .. selectedNPC)
                    end
                else
                    debugPrint("❌ [Dynamic V2] NPC location not found: " .. selectedNPC)
                end
            end)
        end)
        ...
        local dynamicActiveDropdown = DynamicActiveSection:NewDropdown("Dynamic Active Items", "Teleport to active event items & special objects", dynamicActiveItems, function(selectedItem)
            pcall(function()
                -- Scan workspace for the selected item
                local function findItemInWorkspace(itemName)
                    ...
                    return scanFolder(workspace, 0)
                end
                
                local item = findItemInWorkspace(selectedItem)
                if item then
                    local targetPosition
                    if item:FindFirstChild("HumanoidRootPart") then
                        targetPosition = item.HumanoidRootPart.CFrame
                    elseif item:IsA("Model") then
                        targetPosition = item:GetModelCFrame()
                    elseif item:IsA("Part") or item:IsA("MeshPart") then
                        targetPosition = item.CFrame
                    end
                    
                    if targetPosition then
                        gethrp().CFrame = targetPosition + Vector3.new(3, 2, 0)
                        debugPrint("⭐ [Dynamic V2] Teleported to active item: " .. selectedItem)
                    else
                        debugPrint("❌ [Dynamic V2] Could not get position for: " .. selectedItem)
                    end
                else
                    debugPrint("❌ [Dynamic V2] Active item not found: " .. selectedItem)
                end
            end)
        end)
        ...
        RefreshSection:NewButton("🔄 Refresh All Lists", "Manually refresh all dynamic lists", function()
            ...
            debugPrint("🔄 [Dynamic V2] All lists refreshed successfully!")
            debugPrint("🎣 Fishing Zones: " .. #dynamicFishingZones .. " found")
            debugPrint("🌍 Teleport Spots: " .. #dynamicTeleportSpots .. " found")
            debugPrint("👤 NPCs: " .. #dynamicNPCLocations .. " found")  
            debugPrint("⭐ Active Items: " .. #dynamicActiveItems .. " found")
        end)
        ...
        debugPrint("🚀 [Teleport V2] Dynamic teleport system initialized!")
        debugPrint("📊 [Stats] Fishing Zones: " .. #dynamicFishingZones .. " | Spots: " .. #dynamicTeleportSpots .. " | NPCs: " .. #dynamicNPCLocations .. " | Items: " .. #dynamicActiveItems)
